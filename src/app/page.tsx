...
const AICEditor = ({ onSave, rawInput, setRawInput, category, setCategory }: AICEditorProps) => {
  ...
  // AI operations are now routed through our secure /api/ai endpoint

  const handleEdit = async () => {
    if (!rawInput) return;
    setIsEditing(true);
    try {
      const reqRes = await fetch("/api/ai", {
        method: "POST",
      ...
      const response = await reqRes.json();
      setOutput(response.text || "");
      
      // Auto-suggest image prompt
      try {
        const pReqRes = await fetch("/api/ai", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            model: "gemini-2.1-flash",
        ...
        const promptSuggestion = await pReqRes.json();
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      const iReqRes = await fetch("/api/ai", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const response = await iReqRes.json();
